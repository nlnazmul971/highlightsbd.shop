    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            মোট {subscribers?.length || 0} জন subscribe করেছে
          </p>
        </div>
        {(subscribers?.length || 0) > 0 && (
          <button onClick={handleExport} className="luxury-button-outline inline-flex items-center gap-2 text-[10px]">
      ...
      {!subscribers?.length ? (
        ...
        <div className="border border-border">
          <div className="grid grid-cols-[1fr_auto_auto] gap-3 p-3 border-b border-border bg-secondary/30 text-xs text-muted-foreground tracking-wider uppercase">
            <span>Email</span>
            <span>Date</span>
            <span></span>
          </div>
