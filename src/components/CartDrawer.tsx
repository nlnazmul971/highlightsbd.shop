      ...
      <div className="fixed top-0 right-0 h-full w-full sm:w-[420px] bg-background z-50 shadow-2xl animate-scale-in flex flex-col">
                ...
                <div key={idx} className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <button onClick={() => updateQuantity(item.product.id, item.size, item.color, item.quantity - 1)} className="p-1 border border-border hover:bg-accent transition-colors">
                        <Minus size={12} />
                      </button>
                  ...
                  <button onClick={() => removeItem(item.product.id, item.size, item.color)} className="p-1 h-fit hover:opacity-60 transition-opacity">
                    <X size={16} />
                  </button>
                </div>
              ))}
        ...
        {items.length > 0 && (
          <div className="p-6 border-t border-border space-y-4">
            ...
            <Link to="/checkout" onClick={() => setIsCartOpen(false)} className="luxury-button-primary block text-center w-full">
              Checkout
            </Link>
          </div>
        )}
