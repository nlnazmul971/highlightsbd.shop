...
const ProductCard = ({ product }: { product: Product }) => {
  ...
  return (
    <div className="luxury-card animate-fade-in">
      <Link to={`/product/${product.id}`}>
        <div className="relative overflow-hidden aspect-[3/4]">
          ...
          {/* Badges */}
          {discountPercent && (
            <span className="luxury-badge absolute top-3 left-3">-{discountPercent}%</span>
          )}
          {/* Hover overlay */}
          ...
          <div className="absolute bottom-0 left-0 right-0 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
            <button onClick={handleAddToCart} className="w-full luxury-button-primary py-3 text-[10px]">
              Add to Cart
            </button>
          </div>
        </div>
